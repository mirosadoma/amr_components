                        ...
                        <table class="table datatable-basic">
                            <thead>
                                <tr>
                                    ...
                                    <th>@lang('Name')</th>
                                    <th {!! \table_width_head(1) !!}>@lang('City')</th>
                                ...
                                @if ($lists->count())
                                    @foreach ($lists as $item)
                                        <tr>
                                            ...
                                            <td>
                                                @if($item->is_active == 0)
                                                    <a href="{{route('app.clients.is_active', $item->id)}}" class="label label-sm label-danger" title="{{__('Active Client')}}"> <i data-feather="x" stroke-width="7" style="color: red;"></i> </a>
                                                @else
                                                    <a href="{{route('app.clients.is_active', $item->id)}}" class="label label-sm label-success" title="{{__('Un Active Client')}}"> <i data-feather="check" stroke-width="7" style="color: #00b800;"></i> </a>
                                                @endif
                                            </td>
                                            <td> {{$item->created_at->diffForHumans()}} </td>
                                            <td>
                                                @if(permissionCheck('clients.update'))
                                                    {!! editForm('clients', $item) !!}
                                                @endif
                                                @if(permissionCheck('clients.delete'))
                                                    @if(!request()->has('type') || request('type') != "deleted")
                                                        {!! deleteForm('clients', $item) !!}
                                                    @elseif(request()->has('type') && request('type') == "deleted")
                                                        {!! restoreForm('clients', $item) !!}
                                                        {!! destroyForm('clients', $item) !!}
                                                    @endif
                                                @endif
                                                @if(!permissionCheck('clients.update') && !permissionCheck('clients.delete')) ------- @endif
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
