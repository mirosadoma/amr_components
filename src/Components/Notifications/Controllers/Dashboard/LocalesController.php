...
class LocalesController extends Controller {
    ...
    public function index() {
        if (!permissionCheck('notifications.locales')) {
            return abort(403);
        }
        $Breadcrumb = [
            [
            ...
            [
                'name'  =>  __('Locales'),
                'route' =>  'notifications.locales.index',
            ],
        ];
        $Button = [];
        return view('Notifications_Dashboard::locales', get_defined_vars());
    }
    ...
    public function store(Request $request) {
        ...
        file_put_contents(app_path() . '/Components/Notifications/Views/Lang/ar.json', json_encode($data, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
        return 1;
    }
    ...
    public function show($locale) {
        ...
        $locals = json_decode(trim(file_get_contents(app_path() . '/Components/Notifications/Views/Lang/ar.json')));
        $locle = [];
